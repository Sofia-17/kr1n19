...
public:
	...
	ComplexNumber operator+(const double &x) {
		...
		result.Im=Im;
		return result;}
	ComplexNumber operator+(const int &x) {ComplexNumber result; 
	result.Re=Re+x; 
	result.Im=Im; 
	return result;}
	ComplexNumber operator-(const ComplexNumber &x) {
		...
		result.Im=Im-x.Im; 
		return result;}
	ComplexNumber operator-(const double &x) {ComplexNumber result; 
		result.Re=Re-x; 
		result.Im=Im; 
		return result;}
...
public:
	...
	CComplexVector(const int &n){
		this->n=n; v=new ComplexNumber[this->n];}
	...
	void CopyOnly(const CComplexVector &rhs){
		if(this!=&rhs){
			v=new ComplexNumber[n=rhs.n];memcpy(v,rhs.v,n*sizeof(ComplexNumber));
			}
		}
	void CopyOnly(const ComplexNumber *rhs, const int &n){
		this->n=n;
		memcpy(v=new ComplexNumber[this->n],rhs,(n)*sizeof(ComplexNumber));
	}                                                  
