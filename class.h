...
public:
	...
	CComplexVector(const int &n){
		this->n=n; 
		v=new ComplexNumber[this->n];
	}
	...
	CComplexVector &operator=(const CComplexVector &rhs){if(this!=&rhs){
		Clean();CopyOnly(rhs);
		} 
	return *this;}
	...
	void Conjugation(){
		for(int i=0;i<n;i++)v[i].Conjugation();
	}
	...
	void CopyOnly(const CComplexVector &rhs){if(this!=&rhs){
		v=new ComplexNumber[n=rhs.n];
		memcpy(v,rhs.v,n*sizeof(ComplexNumber));
		}
	}
	void CopyOnly(const ComplexNumber *rhs, const int &n)
	{this->n=n;
	memcpy(v=new ComplexNumber[n], rhs, (n)*sizeof(ComplexNumber));
	}                                                  
...
//using namespace CComplexVector;
...
public:
	...
	CComplexVector1(const CComplexVector &rhs): CComplexVector(rhs){}
...
public:
	...
	CComplexVector2(const ComplexNumber *rhs, const int &n){
		this->n=n;
		CopyOnly(rhs,n);
	}
	CComplexVector2(const CComplexVector &rhs): CComplexVector(rhs){}
